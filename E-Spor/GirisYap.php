...
<?php
  include("Baglanti.php");
    ...
    if(isset($_POST["email"],$_POST["sifre"])){
      ...
      $EMail=($_POST["email"]);
      $Sifre=($_POST["sifre"]);
      $sec="SELECT * FROM `kullanicilar` WHERE Email='$EMail' && Sifre='$Sifre'";
      $sonuc=$baglan->query($sec);
      if($sonuc->num_rows>0){
          echo "<script>alert('basarıyla giris Yapıldı')</script>";
          
      }
      else{
          echo "<script>alert('Email ya da sifre hatalı ')</script>";
      }
